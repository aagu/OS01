...
typedef struct
{
    ...
    int8 is_hole;   // 1 if this is a hole. 0 if this is a block.
...
typedef struct
{
    ...
    int8 supervisor;     // Should extra pages requested by us be mapped as supervisor-only?
    int8 readonly;       // Should extra pages requested by us be mapped as read-only?
} heap_t;

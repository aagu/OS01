...
| `kernel/include/kernel/log.h` | **Updated** — already exists on x86_64 and defines `log_err`/`log_warn`/`log_info` as `do { ... } while(0)` macros that expand to per-arch function calls (`_log_write`-style). The spec extends this header so the macro form is the single source of truth for every caller on every architecture. Concretely, `kernel/log.h` declares the macros (e.g. `#define log_err(...) _log_err_impl(__VA_ARGS__)`) and the corresponding per-arch function prototypes. Callers (`pmm.c`, etc.) include this single header — no `kernel/arch/log.h` dispatch, no per-arch `#ifdef`s, no `#undef log_err log_warn log_info` dance. The macro form is the same on every architecture; only the underlying function implementation is per-arch. |
| `kernel/arch/aarch64/log_impl.c` | **NEW** — per-arch implementation of `_log_err_impl`/`_log_warn_impl`/`_log_info_impl` (and any other `_log_*_impl` symbols declared by `kernel/log.h`). The aarch64 implementations vsnprintf the variadic args into a static 256-byte buffer and call `kputs(buf)` to emit on the pl011 UART. **This replaces the previous `kernel/include/kernel/arch/aarch64/boot_log.h`/`kernel/include/kernel/arch/log.h` dispatch approach.** The static inline functions in `boot_log.h` are no longer the API surface; the macros in `kernel/log.h` are. `boot_log.h` may stay as a low-level helper (for the `ram.c` fatal path which uses `log_err` directly without going through the variadic form), but no spec-changed TU consumes it. |

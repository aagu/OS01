...
- Changing the 2 MiB allocation granule, the `bits_map` /
  `pages_struct` / `zones_struct` **struct layout**, or any consumer
  of those struct fields. (The arrays are sized and indexed by
  physical-page number; see the rewrite in §"`pmm_init` rewrite" for
  the non-zero-start indexing fix that allows aarch64 RAM starting
  at e.g. `0x40000000`. This is an indexing change, not a layout
  change.)
...
- `kernel/memory/pmm_arch.c` defines `pmm_arch_normalize` and
  `pmm_arch_zone_split` as **weak** symbols. The weak default
  `pmm_arch_normalize` returns 0 (fatal-no-ranges); the weak
  default `pmm_arch_zone_split` returns `SIZE_MAX`. These defaults
  contain no architecture-specific symbols, so they link cleanly on
  every architecture.
- `kernel/arch/x86_64/pmm_arch.c` defines both as **strong** symbols:
  ...
  returns `0x100000000ULL` (4 GiB threshold).
...
```c
/* kernel/memory/pmm_arch.c — weak defaults only. NO x86-specific
 * references here. The default pmm_arch_normalize returns 0
 * (fatal-no-ranges); the default pmm_arch_zone_split returns
 * SIZE_MAX (no unmapped zones). */
__attribute__((weak))
...
classified `ZONE_UNMAPPED`. The weak default returns `SIZE_MAX` so no
zone is unmapped.

The weak default contains **no architecture-specific symbols**, so it
compiles and links cleanly on every architecture (including aarch64
under `-nostdlib`). Each architecture provides its **strong** override:

- `kernel/arch/x86_64/pmm_arch.c` defines both functions as strong
  symbols: `pmm_arch_normalize` translates E820 entries;
  `pmm_arch_zone_split` returns `0x100000000ULL` (4 GiB threshold,
  the historical x86_64 policy).
- `kernel/arch/aarch64/pmm_arch.c` defines only `pmm_arch_normalize`
  as a strong override; it does **not** override
  `pmm_arch_zone_split`, so the weak default (`SIZE_MAX`) applies.

This model is critical for aarch64: the previous round-5 design had
the weak default as a "byte-for-byte x86_64 implementation", but the
aarch64 build compiles `kernel/memory/pmm_arch.c` (via
`KERNEL_C_SOURCES`) and would have unresolved relocations for
`_text`/`_edata`/`_binary_arch_x86_64_trampoline_bin_{start,end}`/
`X86_64_HANDOFF_*` — none of which the aarch64 linker provides. The
weak default must therefore contain only arch-neutral code.
...
The strong per-arch override is the **only** definition on x86_64;
"forgetting" to include it correctly triggers the
`[smp] FATAL: pmm_arch_normalize returned no ranges` halt (since the
weak default returns 0), not a silent fallback.
...
The new body is a uniform walk over `MEMORY_RANGE[]`. **Critical
non-x86 fix**: `pages_struct` and `bits_map` are sized for the
**total 2 MiB-page count of RAM** (`TotalMem / MEMORY_RANGE_GRANULE`)
but each `pages_group` is **RAM-relative** — offset from the lowest
granule-aligned RAM address, not from absolute physical zero. On
x86_64 (where RAM starts at 0) this is a no-op; on aarch64 (where
the first RAM range starts at e.g. `0x40000000`) the index shift
is `start >> 21` minus `(lowest_ram >> 21)`. Without this fix,
`pages_struct + (start >> 21)` on aarch64 would land outside the
array allocated for `TotalMem/2MiB` pages.

The x86_64-specific `pages_struct[0] = phys 0` quirk is preserved
as a pre-loop step that runs only when the very first RAM page
sits at physical address zero (the historical x86_64 layout). On
aarch64 the first RAM page is at `0x40000000`, so the quirk is a
no-op.
...
2. Iterate scratch_ranges; sum (phys_end - phys_start) for MEMORY_TYPE_RAM
   into TotalMem. Compute pages_total = TotalMem / MEMORY_RANGE_GRANULE.
   Also compute lowest_ram = MIN over MEMORY_TYPE_RAM ranges of
   round_down(phys_start, MEMORY_RANGE_GRANULE) and highest_ram = MAX
   over MEMORY_TYPE_RAM ranges of round_up(phys_end, MEMORY_RANGE_GRANULE).
   Compute **ram_span_pages = (highest_ram - lowest_ram) /
   MEMORY_RANGE_GRANULE** — this is the **array-size metric**, used
   below to size `pages_struct` and `bits_map` for non-contiguous RAM.
   For contiguous RAM (the common case on x86_64 and current QEMU
   virt aarch64), ram_span_pages == pages_total. For sparse RAM
   (future ACPI SRAT boards), ram_span_pages > pages_total.
   Fatal if no RAM ranges.
3. Sanity-check TotalMem > 0 and pages_total > 0; fatal otherwise.
4. Allocate bits_map, pages_struct, zones_struct from PMMngr.start_brk
   ...
   sizes (note: arrays are sized by ram_span_pages, not pages_total,
...
5. For each MEMORY_TYPE_RAM range:
   ...
   b. create one Zone { start, end, page_free_count = (end-start)/granule,
      pages_group = pages_struct + ((start - lowest_ram) >> PAGE_2M_SHIFT),
   ...
   c. for j in 0..pages_length: pages_group[j] = { zone=z, phy_address =
      ...
      toggle bits_map bit free. The bits index is also RAM-relative:
      (p->phy_address - lowest_ram) >> 21.
6. If pages_struct[0].phy_address == 0 (legacy x86_64 layout):
   ...
   (Behaviorally equivalent to existing pmm.c:212-215; the
   pseudo-code matches because step 4 zeroes the array, so a plain
...
Step 5b's `pages_group = pages_struct + ((start - lowest_ram) >> 21)`
is the critical aarch64 fix. Step 5c's bit index `(p->phy_address -
lowest_ram) >> 21` keeps `bits_map` compact (sized by `TotalMem`, not
by `max_phys`).
...
- Invalid handoff / unsupported `memory.format` / zero `entry_count` /
  per-format `entry_size` minimum violated: fatal with
  `[smp] FATAL: pmm_init invalid handoff` and halt BSP with
  interrupts disabled. Per-format minimums are: E820 requires
  `entry_size >= sizeof(struct E820_ENTRY) = 20`; UEFI_RAW requires
  `entry_size >= 32`. A single global `entry_size < 32` check would
  reject every x86_64 boot, because the x86 UEFI loader sets
  `ctx->memory.entry_size = sizeof(struct E820_ENTRY)`.
...
| `kernel/include/kernel/log.h` | **Updated** — already exists on x86_64 with a level-gated infrastructure: `log(level, fmt, ...)` core macro, `log_debug` (NDEBUG-gated), `g_log_level` / `log_set_level` / `log_get_level` (runtime level gate), and the convenience `log_err` / `log_warn` / `log_info` macros. The spec extends this header so the **convenience macros** (`log_err` / `log_warn` / `log_info`) are the single source of truth for every caller on every architecture. **The chosen macro form is gated**: `#define log_err(...) do { if (LOG_ERR &lt;= g_log_level) _log_err_impl(__VA_ARGS__); } while(0)` (and likewise for `log_warn`/`log_info` with `LOG_WARN`/`LOG_INFO`). The naive unwrapped form `#define log_err(...) _log_err_impl(__VA_ARGS__)` is explicitly **not** used because it would bypass the `g_log_level` gate. The gate is preserved inside the macro so user-visible level-gating behaviour is unchanged from today. The existing **level-gated infrastructure stays untouched** on its current `_log_write(level, fmt, ...)` path: `log()`, `log_debug`, `g_log_level`, `log_set_level`, `log_get_level` continue to use `_log_write`. The two mechanisms coexist (callers using `log_info(...)` go through `_log_info_impl`; callers using `log(LOG_INFO, fmt, ...)` go through `_log_write`). No `#undef log_err log_warn log_info` dance; no `kernel/arch/log.h` dispatch header. The macro form is the same on every architecture; only the underlying function implementation is per-arch. The function prototypes `_log_err_impl`, `_log_warn_impl`, `_log_info_impl` are declared in `kernel/log.h` alongside the macros (unconditional; every build links exactly one of the two per-arch implementations). |
| `kernel/arch/aarch64/log_impl.c` | **NEW** — per-arch implementation of `_log_err_impl` / `_log_warn_impl` / `_log_info_impl` (declared in `kernel/log.h`). The aarch64 implementations **do not expand format specifiers** — they call `kputs(fmt)` directly and ignore any extra variadic args. This mirrors `printk_stub.c`'s plain-string approach and avoids pulling in libc (aarch64 links `-nostdlib`; see the `printk_stub.c` row for the same constraint). Today the aarch64 build's only `log_err` / `log_warn` / `log_info` callers all pass plain string literals with no format specifiers (`ram.c:52` `log_err("UEFI-A64: RAM map invalid\n")`, the smoke-test `log_info("UEFI-A64: pmm alloc smoke OK\n")`, the new `pmm.c` private-helper replacements); the x86_64 drivers (`e1000.c`, `virtio-net.c`, `pci.c`, `socket.c`, `net.c`) use format-specifier calls but are not built on aarch64. If a future aarch64 caller needs specifiers, the stub grows a small `number()` helper alongside `kputu` rather than pulling in libc. The `_log_write(level, fmt, ...)` path used by the existing level-gated infrastructure is **unchanged** on x86_64 (where `kernel/kernel/log.c` provides vsnprintf+serial); the aarch64 `_log_write` path stays whatever it is today. |
| `kernel/kernel/log.c` | **Updated** — already defines `_log_write(level, fmt, ...)` doing vsnprintf+serial output. Adds three thin wrappers that forward the convenience macros' variadic args through the existing vsnprintf path. Because the `...` of one variadic function cannot be forwarded directly to another variadic function without `va_list` plumbing, the wrappers use the standard va_list pattern (and `_log_write` is split into a public `...` wrapper and an internal `_log_writev(int level, const char *fmt, va_list args)` helper): ```c void _log_err_impl(const char *fmt, ...) { va_list args; va_start(args, fmt); _log_writev(LOG_ERR, fmt, args); va_end(args); } void _log_warn_impl(const char *fmt, ...) { va_list args; va_start(args, fmt); _log_writev(LOG_WARN, fmt, args); va_end(args); } void _log_info_impl(const char *fmt, ...) { va_list args; va_start(args, fmt); _log_writev(LOG_INFO, fmt, args); va_end(args); } ``` The existing `_log_write(level, fmt, ...)` becomes a thin caller: `void _log_write(int level, const char *fmt, ...) { va_list args; va_start(args, fmt); _log_writev(level, fmt, args); va_end(args); }`. The `_log_writev` body is the old `_log_write` body (vsnprintf into a buffer + serial write). This preserves x86_64 format-specifier behaviour for the existing call sites in `e1000.c:336,395`, `virtio-net.c:73,113,...`, `pci.c:217,235,...`, `socket.c:222`, `net.c:67,115,130,135`. |
...
| `kernel/arch/aarch64/main.c` | Insertion order is pinned: after `aarch64_ram_init(handoff)` returns successfully → set `PMMngr.start_*`/`end_*`/`start_brk` prelude (5 lines) → `pmm_init(handoff)` → `#if OS01_SELFTEST` smoke-test block (alloc/free roundtrip) → `dtb_init(handoff)` → `gic_init(handoff)` → `smp_boot_aps(handoff)` → `arch_tick_start()` → halt. The smoke-test block sits between `pmm_init` and `dtb_init` so it runs only when `pmm_init` is known-good, and `dtb_init`/`gic_init`/`smp_boot_aps` (the first allocator consumers on aarch64) follow it. **The existing `#include <kernel/arch/aarch64/boot_log.h>` is replaced by `#include <kernel/log.h>`** to use the unified macro API; `boot_log.h` static inlines (`log_err` etc.) would otherwise collide with `kernel/log.h`'s macros of the same name. The `ram.c` consumer keeps its `boot_log.h` include unchanged — `ram.c` is a legacy caller that the spec does not modify (its `log_err("UEFI-A64: ...")` continues to resolve to `boot_log.h`'s static inline). |
...
For the **AArch64 adapter**, the host runner compiles
`kernel/arch/aarch64/pmm_arch.c` (the strong override) plus the
weak default `kernel/memory/pmm_arch.c` (which is now arch-neutral
and links cleanly on aarch64). The aarch64 host runner needs **no
x86_64 stubs** (no `_text`, no `_edata`, no trampoline blob, no
`X86_64_HANDOFF_*`) because the weak default no longer references
them.

The runner must link `kernel/arch/aarch64/ram.c` AND
`kernel/arch/aarch64/ram_core.c` to obtain the four aarch64_ram_*
symbols:

- `aarch64_ram_init` and `aarch64_ram_map_get` are defined in
  `ram.c` (the boot-time wrapper, lines 69 and 232);
- `aarch64_ram_normalize` and `aarch64_ram_publish_once` are
  defined in `ram_core.c` (the pure normalizer, lines 287 and 442).

Linking only `ram_core.c` (as the round-5 spec claimed) leaves
`aarch64_ram_init` and `aarch64_ram_map_get` unresolved.
...
- a new `OS01_SELFTEST`-gated block in `kernel/arch/aarch64/main.c`
  ...
  #endif
  ```
  The `log_info` / `log_err` macros come from the unified
  `kernel/log.h` (per-arch `_log_*_impl` via `kernel/kernel/log.c` on
  x86_64, via `kernel/arch/aarch64/log_impl.c` on aarch64).
...
established `DEGRADED` regression evidence. The `tests/systest.py`
and `tests/nettest.py` files referenced in earlier spec revisions do
not exist; the actual entry points are `tests/run_test.py systest` and
`tests/run_test.py network`. The kernel artifact target is
`kernel.bin` (or the existing `make kernel.bin` rule), not the
non-existent `x86_64-kernel` target from earlier spec revisions.

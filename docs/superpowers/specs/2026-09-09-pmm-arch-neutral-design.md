...
| `kernel/include/kernel/log.h` | **Updated** — already exists on x86_64 with a level-gated infrastructure: `log(level, fmt, ...)` core macro, `log_debug` (NDEBUG-gated), `g_log_level` / `log_set_level` / `log_get_level` (runtime level gate), and the convenience `log_err` / `log_warn` / `log_info` macros. The spec extends this header so the **convenience macros** (`log_err` / `log_warn` / `log_info`) are the single source of truth for every caller on every architecture. Concretely, the convenience macros are rewritten to expand directly to per-arch function calls: `#define log_err(...) _log_err_impl(__VA_ARGS__)`. The existing **level-gated infrastructure stays untouched** on its current `_log_write(level, fmt, ...)` path: `log()`, `log_debug`, `g_log_level`, `log_set_level`, `log_get_level` continue to use `_log_write`. The two mechanisms coexist (callers using `log_info(...)` go through `_log_info_impl`; callers using `log(LOG_INFO, fmt, ...)` go through `_log_write`). **Behavior change to flag**: after the rewrite, the convenience macros **bypass the `g_log_level` gate** — `log_err`/`log_warn`/`log_info` always emit, regardless of `log_set_level()`. Today they route through `log(level, ...)` and respect the gate. Two options: (a) preserve the gate by wrapping each macro: `#define log_err(...) do { if (LOG_ERR &lt;= g_log_level) _log_err_impl(__VA_ARGS__); } while(0)`; (b) document the behavior change in a one-line note here and accept that `log_set_level(LOG_ERR)` no longer suppresses `log_info`/`log_warn` calls (only the explicit `log(LOG_INFO, ...)` path remains gated). The spec picks (a) — the gate is preserved inside the macros so the user-visible level-gating behaviour is unchanged. No `#undef log_err log_warn log_info` dance; no `kernel/arch/log.h` dispatch header. The macro form is the same on every architecture; only the underlying function implementation is per-arch. |
| `kernel/arch/aarch64/log_impl.c` | **NEW** — per-arch implementation of `_log_err_impl` / `_log_warn_impl` / `_log_info_impl` (and any other `_log_*_impl` symbols declared by `kernel/log.h`). The aarch64 implementations **do not expand format specifiers** — they call `kputs(fmt)` directly and ignore any extra variadic args. This mirrors `printk_stub.c`'s plain-string approach and avoids pulling in libc (aarch64 links `-nostdlib`; see the `printk_stub.c` row for the same constraint). Today the aarch64 build's only `log_err` / `log_warn` / `log_info` callers all pass plain string literals with no format specifiers (`ram.c:52` `log_err("UEFI-A64: RAM map invalid\n")`, the smoke-test `log_info("UEFI-A64: pmm alloc smoke OK\n")`, the new `pmm.c` private-helper replacements); the x86_64 drivers (`e1000.c`, `virtio-net.c`, `pci.c`, `socket.c`, `net.c`) use format-specifier calls but are not built on aarch64. If a future aarch64 caller needs specifiers, the stub grows a small `number()` helper alongside `kputu` rather than pulling in libc. The `_log_write(level, fmt, ...)` path used by the existing level-gated infrastructure is **unchanged** on x86_64 (where `kernel/kernel/log.c` provides vsnprintf+serial); the aarch64 `_log_write` path stays whatever it is today. |
| `kernel/kernel/log.c` | **Updated** — already defines `_log_write(level, fmt, ...)` doing vsnprintf+serial output. Adds three thin wrappers: `_log_err_impl(fmt, ...) { _log_write(LOG_ERR, fmt, ##__VA_ARGS__); }`, `_log_warn_impl(fmt, ...) { _log_write(LOG_WARN, fmt, ##__VA_ARGS__); }`, `_log_info_impl(fmt, ...) { _log_write(LOG_INFO, fmt, ##__VA_ARGS__); }`. These forward the convenience macros' variadic args through the existing vsnprintf path, preserving x86_64 format-specifier behaviour (used by `e1000.c:336,395`, `virtio-net.c:73,113,...`, `pci.c:217,235,...`, `socket.c:222`, `net.c:67,115,130,135`). The function prototypes are declared in `kernel/log.h` alongside the macros. |

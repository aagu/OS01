...
| `kernel/arch/aarch64/log_impl.c` | **NEW** — per-arch implementation of `_log_err_impl` / `_log_warn_impl` / `_log_info_impl` (and any other `_log_*_impl` symbols declared by `kernel/log.h`). The aarch64 implementations **do not expand format specifiers** — they call `kputs(fmt)` directly and ignore any extra variadic args. This mirrors `printk_stub.c`'s plain-string approach and avoids pulling in libc (aarch64 links `-nostdlib`; see the `printk_stub.c` row for the same constraint). Today every caller of `log_err` / `log_warn` / `log_info` passes a plain string literal with no format specifiers (`ram.c:52` `log_err("UEFI-A64: RAM map invalid\n")`, the smoke-test `log_info("UEFI-A64: pmm alloc smoke OK\n")`, the new `pmm.c` private-helper replacements); if a future caller needs specifiers, the stub grows a small `number()` helper alongside `kputu` rather than pulling in libc. The `_log_write(level, fmt, ...)` path used by the existing level-gated infrastructure is **unchanged** on x86_64 (where `kernel/kernel/log.c` provides vsnprintf+serial); the aarch64 `_log_write` path stays whatever it is today (`boot_log.h` consumers, none in this spec). |

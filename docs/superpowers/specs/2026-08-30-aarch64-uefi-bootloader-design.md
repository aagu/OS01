...
**Date:** 2026-08-30  
**Status:** revised after architecture review; implementation pending approval
...
- A single-BSP (`-smp 1`) phase-1 smoke path.
...
- AArch64 user mode, UEFI/PSCI secondary bring-up, drivers, and storage
  mounting.  The existing BSP timer interrupt remains in scope because it is
  required for the single-core tick smoke test.
...
```text
qemu-system-aarch64 -M virt,gic-version=2 -cpu cortex-a53 -smp 1 -m 512M \
...
The baseline uses the local 64 MiB combined `QEMU_EFI.fd` as a writable pflash
copy.  If a distribution instead supplies separate code-only and variable
firmware images, the QEMU target must attach the read-only `*_CODE.fd` and a
writable per-build `*_VARS.fd` as separate pflash devices; it must not write
the code image.
...
```text
x0 = 0x401e0000        // physical pointer to a validated boot_context
PC = ELF e_entry       // current value 0x40080000
```

`boot_context` gains a fixed `magic`, ABI version, and size.  `aarch64_main`
is intentionally a dual-mode entry during migration:

- `x0 == 0x401e0000` selects the UEFI handoff and its magic/version/size must
  validate before any field is used; and
- every other `x0` value retains the direct `-kernel` ABI: zero selects the
  existing synthetic-DTB fallback, while a nonzero value is the direct-boot
  FDT pointer.

Thus the fixed handoff address, rather than an unsafe probe of an arbitrary
pointer, is the mode discriminator.  `head.S` preserves `x0` while enabling
the existing mappings, then passes it to
`aarch64_main(const struct boot_context *)`.  `make run-aarch64` remains the
direct-boot regression target; `make run-aarch64-uefi` is the new UEFI target.
Neither replaces the other in this milestone.
...
- an optional FDT found through the UEFI configuration tables.  Before
...
format.  It conveys descriptor bytes and numeric UEFI type/attribute values,
not UEFI headers or UEFI ABI declarations.  A later core-init slice is
responsible for defining the corresponding fixed-width numeric constants and
turning that raw map into the architecture-neutral allocator representation.

### MMU and cache transition

The current `head.S` assumes entry with stage-1 translation disabled, then
installs its own low/high mappings without an inherited-TLB invalidation.  The
UEFI loader must therefore establish the same bare-entry state before it
branches to `e_entry`: at EL1 it masks asynchronous exceptions, disables
`SCTLR_EL1.M`, orders the change with `DSB`/`ISB`, invalidates EL1 stage-1 TLB
entries with `TLBI VMALLE1`, and performs the required barriers.  The handoff
stub doing this runs from an identity-mapped physical address and branches to
the physical ELF entry only after the transition; it is not permitted to
assume that the PE/COFF loader image itself remains executable after `M=0`.

The implementation must verify this transition on AAVMF and fail before the
jump if it cannot provide that identity-mapped execution path.  Thus `head.S`
continues to receive the direct-boot contract (MMU off, no inherited stage-1
TLB state), rather than relying on undocumented firmware mappings.

### Phase-1 execution policy

The UEFI target is deliberately BSP-only.  Once it recognizes the validated
UEFI handoff, `aarch64_main` initializes the GIC/timer and emits the handoff
marker, but skips `smp_boot_aps()`.  This is required even with `-smp 1` so
the UEFI acceptance path does not accidentally inherit a future DTB default
or claim an SMP benchmark.  The retained direct target keeps its existing SMP
behavior and is checked separately by its regression smoke test.
...
1. Call `GetMemoryMap()` into the pre-reserved handoff buffer after reserving
   enough space for the context, copied FDT, and map.
...
4. If `ExitBootServices()` reports an invalid map key, retry from step 1 using
   the same preallocated buffer and revalidate that the newly returned map
   still fits.  Any other failure is reported while boot services are still
   available and terminates the boot attempt.
...
validation/loading, GOP/config-table discovery, the UEFI map,
`ExitBootServices()`, and the transition to the bare kernel-entry state.  The
kernel owns page-table activation and all later initialization.  No UEFI
headers or UEFI ABI types cross into the kernel; raw map values are data under
the explicitly documented boot-context format.
...
4. A headless AAVMF QEMU boot at `-smp 1` emits a PL011-visible UEFI-handoff
   marker from the kernel, followed by the existing AArch64 phase-1 banner and
   at least one timer tick.  It must not invoke `smp_boot_aps()` or claim the
   four-core spinlock benchmark passed.  UEFI `ConOut` text is not accepted as
   evidence because `-display none` need not route it to `-serial stdio`.
...
7. The direct `make run-aarch64` target still reaches its existing phase-1
   banner after the dual-mode entry change.

## Follow-up slices

Once this path is stable, the next design/implementation slice will add PSCI
CPU_ON secondary bring-up for AAVMF (including the EL2 timer/SMC trap
preconditions), then normalize the stored UEFI map, select the generic
PMM/VMM bootstrap, and make both x86_64 and AArch64 enter
`kernel_main(const struct boot_context *)`.  Those are intentionally separate
so UEFI loading, PSCI/SMP, and generic-kernel initialization failures retain
distinct, diagnosable boundaries.

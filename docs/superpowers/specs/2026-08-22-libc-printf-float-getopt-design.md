# libc 兼容性修复设计（printf 浮点 + getopt）— 修订版 v11

- 日期：2026-08-22（v11 修订）
...
### 0.11 补充修订（v11，本轮）
- **P1（大写非有限）**：明确 `%F`/`%E`/`%G` 的非有限输出为 `INF`/`NAN`（大写），与 `%f/%e/%g` 的小写 `inf`/`nan` 区分；新增 `%F`/`%E`/`%G` 的 `inf`/`nan` 用例。
- **P1（统一溢出契约验收）**：新增 `snprintf`/`vsnprintf`/`sprintf`/`vsprintf` 在 `total == SIZE_MAX` 或 `total > INT_MAX` 时返回 `-1` 的验收，并断言有容量目标缓冲仍保持 NUL 终止。
...
- 分支修正（`va_arg` 类型必须与格式严格匹配）：
  ...
  - **`%n` 及其长度修饰符（v12 新增，P1）**：`%n` 把“截至目前的输出字符数（即 `total`）”写入指向的整数。`ll` 解析扩展后必须把 `%n` 的修饰符分支**显式列入**，避免实现者误取参：
    - `%n` → `va_arg(args, int *)`；`%ln` → `long *`；`%lln` → `long long *`（三种均实现，符合 C 语义）。
    - 写入值 = 当前 `total`（已生成字符数，不含 NUL）。
    - 不允许 `%lln` 被误当作未知修饰符原样输出——若选择“仅支持 `%n`/`%ln`”的退化方案则须显式声明，但本设计采用完整实现（含 `%lln`）。
- **验收边界值**（写入 §5 用例）：
...
- 算法（呼应 P2，已冻结具体方案，v4 定稿）：**不采用朴素 `frac*=10`**，亦**不移植 musl `fmt_fp`**（其 `long double`/`math.h` 依赖过重）。改为 **自包含 `double`-only 浮点格式化器**，零 `math.h`/`long double` 依赖：
  ...
  - **大整数支撑（满足 P0：纯 `uint64_t` 不足）**：`%f` 大指数（`DBL_MAX`≈1e308）、`%g`/高精度 `%.100f`、以及 10 的幂缩放中间值都超出 64 位，故**引入定容大整数（bigint）**——定长字数组（如 80×`uint32_t`≈2560 bit，足以容纳任意有限 `double` 的完整十进制展开至 ~770 位），提供 `init/adc/sub/mul_small/divmod_small/mul_pow10`。**明确支持上界**：
    ...
    - 非有限：`inf`/`nan` 走专用分支（文本 + 可选符号），不经 bigint；**大小写须与转换符一致**：`%f/%e/%g` → `inf`/`nan`（小写），`%F/%E/%G` → `INF`/`NAN`（大写）；负号前缀 `-` 仍适用（如 `-inf`/`-INF`）。
  ...
  - **实现位置与链接（满足 P0 跨 TU 调用）**：拆为两个文件：
    ...
    - **精度默认值与 `%g` 规则（满足 P1）**：`vformatter` 对未指定 precision 传负值；`floatconv_render` 必须按以下契约处理（被 `seq`、BusyBox `printf`、普通 `%f/%g` 验收依赖）：
      ...
      - **动态精度 `*` 为负值时视为“未指定”**：`%.*f`、`.*g` 等经 `*` 取得的精度若为负数，`vformatter` 在分发前归一化为“未指定”（负值 → -1），浮点据此回退默认精度 6（**不得当作 `.0`**）；整数精度为负则按“无精度”处理（与未指定等价）。
...
- **`vformatter` 接口（v3 新增）**：
  ...
  - **统一溢出契约验收（v11 新增，P1）**：除既有 `printf`/`vasprintf` 外，须测试 `snprintf`/`vsnprintf`/`sprintf`/`vsprintf` 在 `total == SIZE_MAX` 或 `total > INT_MAX` 时**返回 `-1`**；并断言给定一个**有容量的目标缓冲**时其内容仍被 **NUL 终止**（不写出越界、不残留未终止）。
...
- **浮点非有限大小写（v11 新增，P1）**：`inf`/`nan` 大小写须与转换符一致——`%f/%e/%g` 输出 `inf`/`nan`（如 `inf`、`-inf`、`nan`），`%F/%E/%G` 输出 `INF`/`NAN`（如 `INF`、`-INF`、`NAN`）。新增 `%F`/`%E`/`%G` 的 `inf`、`nan` 用例。
- **动态精度 `*` 负值（v12 新增，P1）**：`%.*f` 传负精度视为未指定 → 回退默认精度 6（如 `printf("%.*f", -1, 1.0)`→`"1.000000"`，**不得当 `.0`**）；`%*.*g` 负精度同理回退默认；整数负精度按无精度处理。

...
{
    // Write superblock (1024 bytes at byte offset 1024 = sector 2)
    if (block_device_write(fs->dev, 2, 2, &fs->sb_raw) != 0)
        return -1;
        ...
        for (uint32_t byte_idx = 0; byte_idx < bit_count / 8; byte_idx++) {
            ...
            for (int bit = 0; bit < 8; bit++) {
                if (!(buf[byte_idx] & (1u << bit))) {
                    buf[byte_idx] |= (1u << bit);
                    uint32_t block = g * blocks_in_group + byte_idx * 8 + bit + 1;
...
static void free_block(ext2_fs_t *fs, uint32_t block)
{
    ...
    uint32_t index = (block - 1) % fs->blocks_per_group;
                            ...
                            struct vfs_node *newdir, const char *newname)
{
    ...
    if (exists == 0) {
        ...
        if (exist_type == 2 /* EXT2_FT_DIR */) {
            ...
            free_inode(fs, exist_ino);  // simplified; full rmdir would free blocks
        } else {
...
int ext2_selftest_block_alloc(void)
{
    ...
    uint32_t bitmap_block = fs->bgdesc_table[g].bg_block_bitmap;
    uint32_t bgdesc_block_num = fs->bgdesc_block + g * sizeof(ext2_bgdesc_t) / fs->block_size;

    // Save
    uint8_t save_bitmap[4096], save_bgdesc[4096], save_sb[1024];
    ext2_read_block(fs, bitmap_block, save_bitmap);
    ext2_read_block(fs, bgdesc_block_num, save_bgdesc);
    ...
    if (blk == 0) {
        ...
        ext2_write_block(fs, bgdesc_block_num, save_bgdesc);
...
int ext2_selftest_inode_alloc(void)
{
    ...
    // Also save one inode table block (the one that will be written)
    uint32_t itable_blk_off = 0;
    uint8_t save_itable[4096];
    ext2_read_block(fs, inode_table_start + itable_blk_off, save_itable);
    ...
    if (ino == 0) {
        ...
        block_device_write(fs->dev, 2, 2, save_sb);
        ext2_write_block(fs, inode_table_start + itable_blk_off, save_itable);
        spin_unlock(&fs->lock); return -1;
    }
...
int ext2_selftest_dirent_roundtrip(void)
{
    ...
    // Save root inode table entry
    uint32_t itable_block = fs->bgdesc_table[0].bg_inode_table;
    uint32_t inodes_per_blk = fs->block_size / fs->inode_size;
    uint32_t root_idx = (dir_ino - 1) % inodes_per_blk;
    uint32_t root_itable_blk = itable_block + ((dir_ino - 1) / inodes_per_blk);

    uint8_t save_itable_blk[4096];
    ext2_read_block(fs, root_itable_blk, save_itable_blk);

    // Save inode bitmap + superblock (alloc_inode will touch these)
    uint32_t ibitmap_block = fs->bgdesc_table[0].bg_inode_bitmap;
    uint8_t save_ibitmap[4096];
    ext2_read_block(fs, ibitmap_block, save_ibitmap);
    ...
    ext2_write_block(fs, ibitmap_block, save_ibitmap);
    block_device_write(fs->dev, 2, 2, save_sb);

...
int ext2_selftest_dirent_roundtrip(void)
{
    ...
    // Save inode bitmap/superblock/inode-table BEFORE alloc,
    // but we don't know which group will be used until after alloc_inode.
    // Save all groups' inode bitmaps (worst case rare; typically just group 0).
    // Simplification: save superblock now, save inode resources after alloc.
    ...
    // Now compute which group/block was touched and save those for restore
    uint32_t ag = (new_ino - 1) / fs->inodes_per_group;
    uint32_t aidx = (new_ino - 1) % fs->inodes_per_group;
    uint32_t ibitmap_block = fs->bgdesc_table[ag].bg_inode_bitmap;
    uint32_t itable_blk   = fs->bgdesc_table[ag].bg_inode_table
                            + aidx / inodes_per_blk;
    uint8_t save_ibitmap[4096];
    ext2_read_block(fs, ibitmap_block, save_ibitmap);
    uint8_t save_new_itable[4096];
    ext2_read_block(fs, itable_blk, save_new_itable);
    ...
    ext2_write_block(fs, itable_blk, save_new_itable);
    block_device_write(fs->dev, 2, 2, save_sb);

...
Make `image.mk` parse each file item as `destination=source:mode`, copy to a fresh `rootfs.next/`, apply `chmod`, and emit `$(BUILD_DIR)/image/rootfs.manifest` with tab-separated `file<TAB>destination<TAB>source<TAB>mode` rows. PLAN DEVIATION (busybox copies instead of symlinks): each `ROOTFS_SYMLINKS` entry is staged as a regular-file COPY of the busybox binary and emitted as a `file` row — the OS01 kernel has no symlink support in path lookup/exec (a symlink exec fails ENOEXEC), and the pre-refactor build likewise wrote busybox copies per applet; applet dispatch is by argv[0] basename. Change mkdisk to require `--output <image>`, `--temp-dir <dir>`, and `--rootfs-manifest <file>`; replace every `fopen("disk.img")`, `dd ... of=disk.img`, self-check open, and `/tmp/_mkdisk_*` path with these arguments. While filling the ext2 temporary image, mkdisk reads each manifest row and uses `debugfs write` for `file` entries followed by `debugfs set_inode_field <destination> mode 010<mode>`. Build the host binary as `$(BUILD_DIR)/host-tools/mkdisk`, not `tools/mkdisk`, and require `--temp-dir $(BUILD_DIR)/image/tmp` so parallel profiles cannot collide.
...
Run: `dd if=build/x86_64-clang/image/disk.img of=build/x86_64-clang/image/rootfs.check.ext2 bs=512 skip=133120 count=262144 status=none && debugfs -R 'ls -l /bin' build/x86_64-clang/image/rootfs.check.ext2 | grep -E 'init|tetris|nettest|dmesg' && debugfs -R 'stat /bin/dmesg' build/x86_64-clang/image/rootfs.check.ext2 | grep -F 'Type: regular' && debugfs -R 'stat /bin/dmesg' build/x86_64-clang/image/rootfs.check.ext2 | grep -E 'Size: 19[0-9]{4}' && debugfs -R 'stat /bin/init' build/x86_64-clang/image/rootfs.check.ext2 | grep -F 'Mode:  0755' && debugfs -R 'stat /kernel.bin' build/x86_64-clang/image/rootfs.check.ext2 | grep -F 'Mode:  0644' && debugfs -R 'stat /etc/inittab' build/x86_64-clang/image/rootfs.check.ext2 | grep -F 'Mode:  0644'`.

...
New files (kernel):
  ...
  kernel/include/fs/ext2.h        — create: ext2 on-disk structs, ext2_init()
  ...
  kernel/include/fs/vfs.h         — modify: vfs_ops_t .flags field
...
Modified root files:
  Makefile                        — modify: disk.img target
...
**Files:**
- Modify: `kernel/include/block/blockdev.h:17-19`

- [ ] **Step 1: Add private_data to block_device_t struct**

```c
// kernel/include/block/blockdev.h:17-19 → replace with:
...
Expected: `kernel.bin` created successfully (struct size change, but no dependent .c files reference private_data yet so no functional change)
...
In `kernel/block/blockdev.c`, after line 59 (closing `}` of `block_device_register`), add:
...
In `block_device_register()` (line 52 of blockdev.c), after `dev->write = default_ahci_write;` add:
...
In `kernel/include/block/blockdev.h`, after `block_device_register` declaration (after line 27), add:
...
```c
// kernel/fs/devfs.c:12-18 → replace struct:
...
### Task 4: devfs — register_blkdev and blkdev dispatch

**Files:**
- Modify: `kernel/fs/devfs.c:121-165`
- Modify: `kernel/include/fs/devfs.h:12-13`

- [ ] **Step 1: Add blkdev branch to devfs_read**

Replace `devfs_read` (devfs.c:121-129) with:
...
Replace `devfs_write` (devfs.c:131-139) with:
...
static int devfs_write(vfs_node_t *node, uint64_t offset, uint64_t size, void *buffer)
{
    ...
    if (devices[idx].type == VFS_BLKDEV) {
        block_device_t *bdev = (block_device_t *)devices[idx].private_data;
        if (!bdev || !buffer || size == 0) return 0;
        // Sector-aligned write: read-modify-write for unaligned edges
        uint32_t lba   = (uint32_t)(offset / 512);
        uint32_t count = (uint32_t)((size + 511) / 512);
        if (count == 0) return 0;
        uint8_t *tmp = kmalloc(count * 512);
...
- [ ] **Step 3: Add devfs_register_blkdev() implementation**

After `devfs_register_chrdev` (ends ~devfs.c:226), add:
...
In `kernel/include/fs/devfs.h`, after line 14 (after `devfs_register_chrdev` declaration), add:
...
Run: `make kernel/kernel.bin 2>&1 | tail -5`
Expected: Success (no callers of register_blkdev yet, but devfs_read/devfs_write dispatch compiles clean).
...
### Task 5: GPT — header and CRC32 infrastructure

**Files:**
- Create: `kernel/include/fs/gpt.h`
- Create: `kernel/fs/gpt.c`

- [ ] **Step 1: Create gpt.h with data structures and declarations**
...
typedef struct gpt_partition {
    char            name[40];       // UTF-16LE → ASCII name
    uint8_t         type_guid[16];  // partition type
    uint64_t        start_lba;      // inclusive
    uint64_t        end_lba;        // inclusive
    block_device_t *parent;         // physical disk
    block_device_t *dev;            // partition block device wrapper
} gpt_partition_t;
...
- [ ] **Step 2: Create gpt.c with CRC32 function**
...
// Standard reflected CRC32 (polynomial 0xEDB88320, same as zlib/gzip/PNG)
...
// Forward declarations for partition helpers
    ...
    block_device_t *parent, uint64_t offset_lba, uint64_t length)
{
    ...
    // Name: parent name + partition number, e.g. "hda1"
    char name[16];
    int pnum = block_device_count() + 1;  // partitions after physical disks
    name[0] = '\0';
    strcat(name, parent->name);
    int nlen = strlen(name);
    name[nlen]     = '0' + (char)pnum;
    name[nlen + 1] = '\0';
...
- [ ] **Step 3: Add gpt_scan() to gpt.c**

Append to `kernel/fs/gpt.c`:

```c
// Check if 16 bytes are all zero (empty GUID)
...
// Convert a single UTF-16LE character to ASCII (basic multilingual plane only)
static char utf16le_to_ascii(uint16_t wc)
{
    if (wc < 0x80) return (char)wc;
    return '?';  // non-ASCII → '?'
}

// Extract name from a GPT partition entry (36 UTF-16LE code units at offset 56)
static void gpt_extract_name(const uint8_t *entry, int entry_size, char *out, size_t out_len)
{
    if (entry_size < 128) { out[0] = '\0'; return; }
    const uint8_t *name_field = entry + 56;  // 36 UTF-16LE chars = 72 bytes
    size_t pos = 0;
    for (int i = 0; i < 36 && pos < out_len - 1; i++) {
        uint16_t wc = (uint16_t)name_field[i * 2]
                    | ((uint16_t)name_field[i * 2 + 1] << 8);
        if (wc == 0) break;
        out[pos++] = utf16le_to_ascii(wc);
    }
    out[pos] = '\0';
}

#include <fs/devfs.h>  // for devfs_register_blkdev
...
gpt_info_t *gpt_scan(block_device_t *disk)
{
    ...
    // Read dynamic parameters from header
    ...
    // Sanity check: don't allocate > 1MB for partition entries
    ...
    // Phase 2: Validate header CRC32
    uint32_t crc_saved = hdr_crc_stored;
    memset(hdr + 16, 0, 4);  // zero out CRC field
    uint32_t crc_computed = gpt_crc32(hdr, header_size);
    hdr[16] = (uint8_t)(crc_saved & 0xFF);  // restore (not strictly needed)
    if (crc_computed != crc_saved) {
        debug_block("gpt: header CRC mismatch (computed %#x, stored %#x)\n",
                    crc_computed, crc_saved);
        return NULL;
    }

    // Phase 3: Read partition entry array
    ...
    for (uint32_t i = 0; i < num_entries && info->count < GPT_PARTITION_MAX; i++) {
        ...
        part->parent    = disk;
        gpt_extract_name(entry, entry_size, part->name, sizeof(part->name));

        // Create partition block device wrapper
        part->dev = block_device_create_partition(disk, start_lba, length);
        if (!part->dev) {
        ...
        debug_block("gpt: partition %d '%s' LBA %lu-%lu (%lu sectors)\n",
                    info->count, part->name, start_lba, end_lba, length);
        info->count++;
    }
...
Run: `make kernel/kernel.bin 2>&1 | tail -5`
Expected: Success. gpt.c includes `devfs.h` for `devfs_register_blkdev`. Confirm the include path resolves.
...
### Task 6: ext2 — header file with on-disk structures
...
// ── On-disk superblock (264 bytes + trailing padding to 1024) ──
...
// ── Inode (first 128 bytes of variable-size struct) ────
typedef struct __attribute__((packed)) {
    uint16_t i_mode;
    uint16_t i_uid;
    uint32_t i_size;
...
### Task 7: ext2 — read-only driver implementation

**Files:**
- Create: `kernel/fs/ext2.c`

- [ ] **Step 1: Create ext2.c with mount, read_inode, bmap, and lookup**
...
// ── Read an inode from disk into the output struct ──────
static int ext2_read_inode(ext2_fs_t *fs, uint32_t ino, ext2_inode_t *out)
{
    ...
    // Only copy the first 128 bytes (our struct size)
    memcpy(out, buf + inode_off, sizeof(ext2_inode_t));
    return 0;
}

// ── Map logical block → physical block (direct + single indirect) ─
                          ...
                          uint32_t logical_block)
{
    ...
    if (logical_block < 12 + ptrs_per_block) {
        ...
        uint32_t indirect[1024];  // 4096/4 = 1024 entries max
        if (ext2_read_block(fs, indirect_block, indirect) != 0)
            return 0;
...
// ── Find a directory entry by name (uses name_len, not strcmp) ─
static uint32_t ext2_find_entry(ext2_fs_t *fs, ext2_inode_t *dir,
                                const char *name, uint8_t *out_type)
{
    ...
    for (uint32_t blk_idx = 0; ; blk_idx++) {
        uint32_t phys = ext2_bmap(fs, dir, blk_idx);
        if (phys == 0) break;
...
int ext2_init(block_device_t *dev, ext2_fs_t **out_fs)
{
    *out_fs = NULL;
    ...
    if (block_device_read(dev, 2, 2, sb_buf) != 0) {
        debug_fs("ext2: failed to read superblock\n");
        kfree(fs); return -1;
    }

    ext2_superblock_t *sb = (ext2_superblock_t *)sb_buf;
    if (sb->s_magic != EXT2_MAGIC) {
        debug_fs("ext2: bad magic %#x (expected %#x)\n",
    ...
    // bgdesc starts at block after superblock
    // superblock is at byte 1024 → block 0 if block_size >= 2048, block 1 if 1024
    ...
    for (uint32_t i = 0; i < table_blocks; i++) {
        ...
        {
            debug_fs("ext2: failed to read bgdesc table\n");
        ...
        while (off < fs->block_size) {
            ...
            if (de->inode != 0) {
                if (entry_idx == index) {
                    ...
                    entry->name[nlen] = '\0';
                    entry->ino  = de->inode;
                    entry->size = 0;   // filled by vfs_stat for files
                    if (de->file_type == 2)
                        entry->type = VFS_DIR;
                    else
                        entry->type = VFS_FILE;
...
Run: `make kernel/kernel.bin 2>&1 | tail -5`
Expected: Success. Compiler may warn about unused `ext2_find_entry` — acceptable (used internally, not from VFS ops directly).
...
### Task 8: vfs_ops_t — add per-fs case sensitivity flag

**Files:**
- Modify: `kernel/include/fs/vfs.h:21-48`
- Modify: `kernel/fs/vfs.c:174,190` (__vfs_lookup name comparison)

- [ ] **Step 1: Add flags field to vfs_ops_t**

In `kernel/include/fs/vfs.h`, at the top of `vfs_ops_t` struct (before `.read`):
...
- [ ] **Step 2: Update __vfs_lookup name comparison**

In `kernel/fs/vfs.c`, find the comparison at line 190 (inside `__vfs_lookup`, after `if (ret == 0)`). Change:
...
- [ ] **Step 3: Add .flags to all existing ops definitions**

In `kernel/fs/fat.c` — find `static struct vfs_ops fat_vfs_ops = {` and add `.flags = VFS_OPS_CASE_INSENSITIVE,` as first field.

In `kernel/fs/devfs.c` — find `static struct vfs_ops devfs_ops = {` — no change needed (`.flags = 0` by default, case-sensitive, correct for /dev).

In `kernel/fs/procfs.c` — find the ops struct and add `.flags = 0` explicitly at the top if desired (C zero-initialization handles this, so optional).

- [ ] **Step 4: Build**

Run: `make kernel/kernel.bin 2>&1 | tail -5`
Expected: Success. No changes to behavior for existing code (FAT32 still case-insensitive; devfs/procfs become case-sensitive but their names are all lowercase anyway).

- [ ] **Step 5: Commit**

```bash
git add kernel/include/fs/vfs.h kernel/fs/vfs.c kernel/fs/fat.c kernel/fs/devfs.c kernel/fs/procfs.c
git commit -m "feat(vfs): add per-fs case sensitivity via vfs_ops_t.flags"
```
...
**Files:**
- Modify: `kernel/fs/fat.c:1202` (find the fat32_mount function implementation)
- Modify: `kernel/include/fs/fat.h:99`
- Modify: `kernel/kernel/main.c:182-184` (call site)

- [ ] **Step 1: Change fat32_mount signature**

In `kernel/include/fs/fat.h:99`, change:
...
Find the `fat32_mount` function definition in `kernel/fs/fat.c` (search for `fat32_mount`). Change the function signature, remove any internal `vfs_mount` call, and set `*out_fs = fs` on success.

Use grep to find the exact location first:
Run: `grep -n "fat32_mount\|fat32_init" kernel/fs/fat.c`

Expected: find the function signature near line ~1200. Change:
```c
fat32_fs_t *fat32_mount(block_device_t *dev)
{
```
...
And at the end of the function, change `return fs;` to `*out_fs = fs; return 0;`, and all error `return NULL;` to `return -1;`.

- [ ] **Step 3: Update call site in main.c**

In `kernel/kernel/main.c`, find lines 180-184:
```c
...
This preserves the old fallback behavior — when GPT scan fails, the old single-FAT layout still works.
...
### Task 10: tmpfs — memory filesystem implementation
...
- [ ] **Step 2: Create tmpfs.c — data structures and helper functions**
...
// ── Block metadata struct (kmalloc) ─────────────────────
typedef struct tmpfs_block_ptr {
    struct tmpfs_block_ptr *next;
    uint64_t                blk_idx;
    void                   *page;         // alloc_pages(0) → Phy_To_Virt → 4096B
} tmpfs_block_ptr_t;
...
// ── Helper: grow block chain to cover up to blk_idx ─────
static int tmpfs_grow_to(tmpfs_node_t *tn, uint64_t last_blk)
{
    for (uint64_t i = 0; i <= last_blk; i++) {
        ...
        if (!bp) return -1;
        bp->blk_idx = i;
        bp->page    = Phy_To_Virt(alloc_pages(0));  // 1 page = 4KB
        if (!bp->page) { kfree(bp); return -1; }
...
// ── Helper: trim blocks past new_size ───────────────────
...
// ── Helper: find child in directory by name ─────────────
...
static int tmpfs_vfs_read(struct vfs_node *, uint64_t, uint64_t, void *);
static int tmpfs_vfs_write(struct vfs_node *, uint64_t, uint64_t, void *);
static int tmpfs_vfs_readdir(struct vfs_node *, uint64_t, struct vfs_dirent *);
static struct vfs_node *tmpfs_vfs_create(struct vfs_node *, const char *);
static int tmpfs_vfs_unlink(struct vfs_node *, const char *);
static struct vfs_node *tmpfs_vfs_mkdir(struct vfs_node *, const char *);
static int tmpfs_vfs_rmdir(struct vfs_node *, const char *);
static int tmpfs_vfs_rename(struct vfs_node *, const char *,
                             struct vfs_node *, const char *);
static int tmpfs_vfs_truncate(struct vfs_node *, uint64_t);
...
- [ ] **Step 3: Add read/write implementation**

Append to `kernel/fs/tmpfs.c`:
...
- [ ] **Step 4: Add readdir, create, mkdir, unlink, rmdir, rename, truncate**
                              ...
                              struct vfs_dirent *entry)
{
    ...
    if (index == 0) {
        strcpy(entry->name, ".");
        entry->type = VFS_DIR;
        entry->ino  = (uint32_t)(uintptr_t)d;
        return 0;
    }
    if (index == 1) {
        strcpy(entry->name, "..");
        entry->type = VFS_DIR;
        entry->ino  = (uint32_t)(uintptr_t)(d->parent ? d->parent : d);
        return 0;
    }
    ...
    entry->ino  = (uint32_t)(uintptr_t)child;
    return 0;
}
                                          ...
                                          const char *name)
{
    ...
    if (tmpfs_find_child(d, name) >= 0) return NULL;  // exists
    ...
    // Expand children array via kmalloc+memcpy+kfree (NEVER realloc)
    ...
    // Allocate a VFS node wrapping this tmpfs node
...
static struct vfs_node *tmpfs_vfs_mkdir(struct vfs_node *dir,
                                         const char *name)
{
    // Same as create but type = VFS_DIR
    tmpfs_node_t *d = (tmpfs_node_t *)dir->fs_data;
    if (!d || d->type != VFS_DIR) return NULL;
...
static int tmpfs_vfs_unlink(struct vfs_node *dir, const char *name)
{
    ...
    // Shrink array: move last element to this slot
...
static int tmpfs_vfs_rmdir(struct vfs_node *dir, const char *name)
{
    ...
    tmpfs_node_t *child = d->children[idx];
    if (child->child_count > 0) return -1;  // directory not empty
    kfree(child);
                             ...
                             struct vfs_node *newdir, const char *newname)
{
    ...
    // Check target doesn't exist in newdir
...
### Task 11: kernel_main.c — new storage init sequence

**Files:**
- Modify: `kernel/kernel/main.c:176-212` (storage + filesystem section)
...
- [ ] **Step 2: Replace storage section (lines ~176-190)**

Replace the existing storage+filesystem block with:
...
```c
    ...
    if (!gpt) {
        ...
        // Dual-partition layout: /boot (hda1=FAT32), / (hda2=ext2)
        // hda1 = gpt->partitions[0], hda2 = gpt->partitions[1]
...
Note: Remove the old `vfs_debug_list("/dev")` and `/dev/null` smoke test lines (lines 202-212), or keep them but they'll run after the new sequence.
...
### Task 12: ext2_vfs_ops — expose to external linkage

**Files:**
- Modify: `kernel/fs/ext2.c` — make static → extern

- [ ] **Step 1: Remove `static` from ext2_vfs_ops**

In `kernel/fs/ext2.c`, change `static struct vfs_ops ext2_vfs_ops = {` to:
```c
struct vfs_ops ext2_vfs_ops = {
```

(Or add `extern struct vfs_ops ext2_vfs_ops;` to `ext2.h`. Simpler: declare in ext2.h.)

In `kernel/include/fs/ext2.h`, add before the `#endif`:
```c
#include <fs/vfs.h>
...
Run: `make kernel/kernel.bin 2>&1 | tail -5`
Expected: Success. main.c can reference ext2_vfs_ops.

- [ ] **Step 3: Commit**

```bash
git add kernel/fs/ext2.c kernel/include/fs/ext2.h
git commit -m "fix(ext2): expose ext2_vfs_ops for main.c mount"
```
...
- [ ] **Step 1: Create tools/Makefile with dependency check**
...
- [ ] **Step 2: Create mkdisk.c — header and CRC32**

```c
// tools/mkdisk.c — GPT dual-partition disk image builder
// Build: gcc -Wall -O2 -o mkdisk mkdisk.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
...
#define PART2_START    ((PART1_END + 1 + (ALIGN_LBA - 1)) & ~((uint64_t)(ALIGN_LBA - 1)))
...
// Write little-endian values into a buffer
...
- [ ] **Step 3: Add GPT header and partition entry building**
...
static void build_mbr(uint8_t *sector)
{
    ...
    // Partition entry at offset 446: type 0xEE (GPT Protective)
    sector[446 + 4] = 0xEE;
    sector[510] = 0x55;
    sector[511] = 0xAA;
}

// Build GPT header (LBA 1)
static void build_gpt_header(uint8_t *hdr)
{
    memset(hdr, 0, 92);
    memcpy(hdr, "EFI PART", 8);
    wr32(hdr, 8,  0x00010000);      // revision
    wr32(hdr, 12, 92);              // header_size
    wr64(hdr, 24, 1);               // my_lba
    wr64(hdr, 32, TOTAL_SECTORS - 1); // alternate_lba (backup GPT)
...
// Generate a random GUID (from /dev/urandom or time-based)
...
// Write a UTF-16LE name into a partition entry (max 36 chars)
...
- [ ] **Step 4: Add main() with system() orchestration**
...
int main(int argc, char **argv)
{
    ...
    printf("Building disk.img: FAT32=%dMB ESP + ext2=%dMB root\n", FAT32_SIZE_MB, EXT2_SIZE_MB);

    // ── Phase 1: Write GPT structure ─────────────────────
    FILE *f = fopen("disk.img", "wb");
    if (!f) { perror("fopen disk.img"); return 1; }
    ...
    // CRC32 for GPT header (CRC field = 0 during computation)
    wr32(gpt_hdr, 16, 0);
    ...
    // Pad to start of first partition
    uint64_t pos = 2 + (sizeof(entries) / SECTOR_SIZE);  // LBA 2 + 32 sectors of entries
    while (pos < PART1_START) {
    ...
    // ── Phase 2: Build + inject ESP image ────────────────
    ...
    // ── Phase 3: Build + inject ext2 image ───────────────
    ...
    // Copy all files from fsroot/bin/ to /bin/
    {
        char glob_cmd[1024];
    ...
    printf("Self-check...\n");
    {
        ...
        f = fopen("disk.img", "rb");
        if (f) {
            // Read back GPT header
            fseek(f, SECTOR_SIZE, SEEK_SET);
            fread(buf, 1, 92, f);
            if (memcmp(buf, "EFI PART", 8) == 0)
                printf("  GPT header: OK\n");
            else
                printf("  GPT header: FAIL\n");

            // Read back ext2 superblock
            fseek(f, PART2_START * 512LL + 1024, SEEK_SET);
            fread(buf, 1, 2, f);
            if (buf[56] == 0x53 && buf[57] == 0xEF)
                printf("  ext2 magic: OK\n");
            else
                printf("  ext2 magic: FAIL\n");

            // Read back FAT magic
            fseek(f, PART1_START * 512LL + 510, SEEK_SET);
            fread(buf, 1, 2, f);
            if (buf[0] == 0x55 && buf[1] == 0xAA)
                printf("  FAT32 magic: OK\n");
            else
                printf("  FAT32 magic: FAIL\n");
    ...
    printf("Done: disk.img (%lu MB)\n",
           (unsigned long)((PART2_END + 1) * SECTOR_SIZE / 1024 / 1024));
    return 0;
}
```

- [ ] **Step 4: Test mkdisk build**
...
git commit -m "feat(tools): add mkdisk — GPT dual-partition disk builder"
```

---

### Task 14: Root Makefile — disk.img target update

**Files:**
- Modify: `Makefile:97-112` (disk.img target)

- [ ] **Step 1: Replace disk.img target**

Replace the current `disk.img:` target (lines 97-112) with:
...
- [ ] **Step 2: Add config/fsroot/ to .gitignore**
...
Run: `make disk.img 2>&1 | tail -10`
Expected: `tools/mkdisk` builds, `disk.img` created (~196 MB), self-check shows OK.
...
### Task 15: Integration test — boot with new layout
...
Expected: Successful build, disk.img created.
...
VFS: mounted '/boot'
VFS: mounted '/tmp'
→ init.elf starts successfully
```

- [ ] **Step 3: Test /dev listing**

Once init shell is available, verify:
```
ls /dev
...
If boot succeeds, this task is complete without code changes.

---

### Task 16: SELFTEST — ext2 and tmpfs kernel self-tests
...
In `kernel/fs/ext2.c`, at the bottom, add:
...
{
    // Verify that our EXT2_MAGIC constant is correct
    ASSERT(EXT2_MAGIC == 0xEF53);
    return 0;
}

SELFTEST(test_ext2_superblock_size)
{
    // ext2_superblock_t must map the first 264 bytes correctly
    // s_magic is at offsets 56-57 (byte 1080-1081 in the 1024-byte block at offset 1024)
    ASSERT(sizeof(ext2_superblock_t) >= 264);
    return 0;
}

SELFTEST(test_ext2_inode_size)
{
    // ext2_inode_t must be exactly 128 bytes (standard ext2)
    ASSERT(sizeof(ext2_inode_t) >= 128);
    return 0;
}
#endif
```
...
In `kernel/fs/tmpfs.c`, add:

```c
#ifdef KERNEL_SELFTEST
#include <kernel/selftest.h>

SELFTEST(test_tmpfs_basic)
{
    // tmpfs init is called during boot; verify /tmp is mounted
    struct vfs_node *tmp = vfs_lookup("/tmp");
    ASSERT(tmp != NULL);
    ASSERT(tmp->type == VFS_DIR);
    vfs_node_put(tmp);
    return 0;
}
#endif
...
- [ ] **Step 3: Add GPT SELFTEST**

In `kernel/fs/gpt.c`, add:
...
{
    // Verify CRC32 against known test vector
...
#endif
```
Note: `gpt_crc32` must be non-static (or duplicated with a different name) for the test. Either add `static` inline version or promote `gpt_crc32` to file scope.
...
Expected: Success. Tests register in selftest table.

- [ ] **Step 5: Commit**

```bash
git add kernel/fs/ext2.c kernel/fs/tmpfs.c kernel/fs/gpt.c
git commit -m "test: add SELFTEST for ext2, tmpfs, and GPT CRC32"
```
...
|---|---|
| §1.1-1.4 GPT partition scan | Tasks 1, 2, 5, 11 |
| §2.1-2.4 Devfs blkdev | Tasks 3, 4, 11 |
| §3.1-3.8 ext2 driver | Tasks 6, 7, 12 |
| §4.1-4.5 tmpfs | Task 10 |
| §5.1-5.5 Build system | Tasks 13, 14 |
| §6 Boot sequence | Task 11 |
| §7.1-7.4 VFS (already correct) | No changes needed |
...
**Plan complete and saved to `docs/superpowers/plans/2026-07-11-ext2-tmpfs-gpt.md`.**
